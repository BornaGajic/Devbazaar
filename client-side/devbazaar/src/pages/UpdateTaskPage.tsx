...
const UpdateTaskPage = observer(({ myTaskStore }: UpdateTaskProps) => {

    let { taskId } = useParams<UpdateTaskUrlParams>();

    let [title, setTitle] = useState('');
    let [description, setDescription] = useState('');
    let [minCost, setMinCost] = useState(-1);
    let [maxCost, setMaxCost] = useState(-1);
    ...
    let handleSubmit = (e: SyntheticEvent) => {
        ...
        myTaskStore.updateTask({
            id: taskId,
            description: description.length === 0 ? undefined : description,
            title: title.length === 0 ? undefined : title,
            highPrice: maxCost === -1 ? undefined : maxCost,
            lowPrice: minCost === -1 ? undefined : minCost
        });
